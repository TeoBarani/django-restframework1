...
def comic_create_api_view(request):
    ...
    if _created:
        return Response(data=model_to_dict(_instance), status=status.HTTP_201_CREATED)
    return Response(
        data={"Ya existe un comic con ese valor, el cual debe ser único"},
        status=status.HTTP_400_BAD_REQUEST
    )

@api_view(http_method_names=['GET'])
def comic_list_filtered_api_view(request):
    _queryset = Comic.objects.filter(price__gt=5)
    _data = list(_queryset.values())
    return Response(data=_data, status=status.HTTP_200_OK)

@api_view(http_method_names=['GET'])
def comic_retrieve_filtered_api_view(request):
    instance = get_object_or_404(Comic, title=request.query_params.get('title'))
    return Response(data=model_to_dict(instance), status=status.HTTP_200_OK)